 ...
 * a simple {@link org.hipparchus.ode.events.ODE_Event_Handler#g(org.hipparchus.ode.ODE_State_And_Derivative)
 ...
 * event when its sign changes. The magnitude of the value is almost irrelevant, * it should however be continuous (but not necessarily smooth) for the sake of
 ...
 *  public double g(const ODE_State_And_Derivative state) 
 {
 *    return state.get_state()[0] - target_concentration;
 *  }
 *
 ...
 * smooth as long as no maneuver are performed, depending only on gravity, drag, * third body attraction, radiation pressure. Firing a thruster introduces a
 ...
 *  public double g(const ODE_State_And_Derivative state) 
 {
 *    return (state.get_time() - t_maneuver_start) &lowast; (state.get_time() - t_maneuver_stop);
 *  }
 *
 *  public Action event_occurred(const ODE_State_And_Derivative state, const bool increasing) 
 {
 ...
 *  public double g(const ODE_State_And_Derivative state) 
 {
 *  const std::vector<double> y = state.get_state();
 *    return y[0] - y[1];
 *  }
 *
 *  public Action event_occurred(const ODE_State_And_Derivative state, const bool increasing) 
 ...
 * {@link org.hipparchus.ode.events.ODE_Event_Handler ODE_Event_Handler} and
 * {@link org.hipparchus.ode.ODE_Integrator ODE_Integrator}.
 *
 * <ol>
 *     <li> An event must be detected if the g function has changed signs for longer than
 *     max_check. Formally, if r is a root of g(t), g has one sign on [r-max_check, r) and
 *     the opposite sign on (r, r+max_check] then r must be detected. Otherwise the root
 ...
 *     {@link org.hipparchus.ode.ODE_Integrator#add_event_handler(org.hipparchus.ode.events.ODE_Event_Handler, double, double, int, org.hipparchus.analysis.solvers.Bracketed_Univariate_Solver) adding}
 ...
 *     {@link org.hipparchus.ode.events.ODE_Event_Handler#event_occurred(org.hipparchus.ode.ODE_State_And_Derivative, bool) event_occurred()}
 *     method is called. An event stops occurring when event_occurred() returns or when the
 *     handler's
 *     {@link org.hipparchus.ode.events.ODE_Event_Handler#reset_state(org.hipparchus.ode.ODE_State_And_Derivative) reset_state()}
 *     method returns if event_occured() returned
