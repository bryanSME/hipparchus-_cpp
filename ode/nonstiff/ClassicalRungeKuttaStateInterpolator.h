...
class ClassicalRunge_Kutta_State_Interpolator
    ...
    {
        return ClassicalRunge_Kutta_State_Interpolator(new_forward, new_y_dot_k, new_global_previous_state, new_global_current_state, new_soft_previous_state, new_soft_current_state, new_mapper);
    }
    ...
    protected ODE_State_And_Derivative compute_interpolated_state_and_derivatives(const Equations_mapper mapper, const double time, const double& theta, const double theta_h, const double one_minus_theta_h) 
    {
        ...
        const double one_minus2_theta            = 1.0 - theta * 2.0;
        const double coeff_dot_1                 = one_minus_theta * one_minus2_theta;
        const double coeff_dot_23                =  theta * one_minus_theta * 2;
        const double coeff_dot_4                 = -theta * one_minus2_theta;
        ...
        {
            const double four_theta_2      = theta * theta * 4;
            const double s               = theta_h / 6.0;
            const double coeff1          = s * (four_theta_2 - theta * 9 + 6);
            const double coeff23         = s * (theta * 6 - four_theta_2);
            const double coeff4          = s * (four_theta_2 - theta * 3);
        ...
        {
            const double four_theta       = theta * 4;
            const double s               = one_minus_theta_h / 6.0;
            const double coeff1          = s * (theta * (-four_theta + 5) - 1);
            const double coeff23         = s * (theta * ( four_theta - 2) - 2);
            const double coeff4          = s * (theta * (-four_theta - 1) - 1);
