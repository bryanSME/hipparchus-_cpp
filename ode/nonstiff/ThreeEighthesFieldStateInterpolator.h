...
class Three_Eighthes_Field_State_Interpolator : public Runge_Kutta_Field_State_Interpolator<T>
{
    ...
    /** {@inherit_doc} */
    //override
    protected Three_Eighthes_Field_State_Interpolator<T> create(const Field<T> new_field, const bool new_forward, const std::vector<std::vector<T>> new_y_dot_k, const Field_ODE_State_And_Derivative<T> new_global_previous_state, const Field_ODE_State_And_Derivative<T> new_global_current_state, const Field_ODE_State_And_Derivative<T> new_soft_previous_state, const Field_ODE_State_And_Derivative<T> new_soft_current_state, const FieldEquations_mapper<T> new_mapper)
    {
        return Three_Eighthes_Field_State_Interpolator<T>(new_field, new_forward, new_y_dot_k, new_global_previous_state, new_global_current_state, new_soft_previous_state, new_soft_current_state, new_mapper);
    }
    ...
    protected Field_ODE_State_And_Derivative<T> compute_interpolated_state_and_derivatives(const FieldEquations_mapper<T> mapper, const T time, const T theta, const T theta_h, const T one_minus_theta_h)
    {
        ...
        const T coeff_dot_3 = theta.multiply(0.75);
        const T coeff_dot_1 = coeff_dot_3.multiply(theta.multiply(4).subtract(5)).add(1);
        const T coeff_dot_2 = coeff_dot_3.multiply(theta.multiply(-6).add(5));
        const T coeff_dot_4 = coeff_dot_3.multiply(theta.multiply(2).subtract(1));
        ...
        {
            const T s = theta_h.divide(8);
            const T four_theta_2 = theta.multiply(theta).multiply(4);
            const T coeff1 = s.multiply(four_theta_2.multiply(2).subtract(theta.multiply(15)).add(8));
            const T coeff2 = s.multiply(theta.multiply(5).subtract(four_theta_2)).multiply(3);
            const T coeff3 = s.multiply(theta).multiply(3);
        ...
        {
            const T s = one_minus_theta_h.divide(-8);
            const T four_theta_2 = theta.multiply(theta).multiply(4);
            const T theta_plus1 = theta.add(1);
            const T coeff1 = s.multiply(four_theta_2.multiply(2).subtract(theta.multiply(7)).add(1));
            const T coeff2 = s.multiply(theta_plus1.subtract(four_theta_2)).multiply(3);
            const T coeff3 = s.multiply(theta_plus1).multiply(3);
            const T coeff4 = s.multiply(theta_plus1.add(four_theta_2));
