...
class Three_Eighthes_Field_Integrator : public Runge_Kutta_Field_Integrator<T>
{
     ...
     */
    public Three_Eighthes_Field_Integrator(const Field<T> field, const T step)
    {
    ...
    public std::vector<std::vector<T>> get_a()
    {
        ...
        a[0][0] = fraction(1, 3);
        a[1][0] = a[0][0].negate();
        a[1][1] = get_field().get_one();
        a[2][0] = get_field().get_one();
        a[2][1] = get_field().get_one().negate();
    ...
    /** {@inherit_doc} */
    //override
    protected Three_Eighthes_Field_State_Interpolator<T>
    ...
    {
        return Three_Eighthes_Field_State_Interpolator<T>(get_field(), forward, y_dot_k, global_previous_state, global_current_state, global_previous_state, global_current_state, mapper);
    }
