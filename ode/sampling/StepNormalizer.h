...
/**
 * This class wraps an object implementing {@link ODE_Fixed_Step_Handler}
 * into a {@link ODE_Step_Handler}.
 ...
 * ODE_Fixed_Step_Handler#handle_step handle_step} method of the underlying
 * {@link ODE_Fixed_Step_Handler} object is called at normalized times. The
 ...
 * @see ODE_Fixed_Step_Handler
 * @see _step_normalizer_mode
 * @see Step_Normalizer_Bounds
 */

class Step_Normalizer : ODE_Step_Handler 
{
    ...
    private const ODE_Fixed_Step_Handler handler;

    /** First step state. */
    private ODE_State_And_Derivative first;

    /** Last step step. */
    private ODE_State_And_Derivative last;
     ...
     */
    public Step_Normalizer(const double h, const ODE_Fixed_Step_Handler handler) 
    {
     ...
     */
    public Step_Normalizer(const double h, const ODE_Fixed_Step_Handler handler, const _step_normalizer_mode mode) 
    {
     ...
     */
    public Step_Normalizer(const double h, const ODE_Fixed_Step_Handler handler, const Step_Normalizer_Bounds bounds) 
    {
     ...
     * @param bounds step normalizer bounds setting to use
     */
    public Step_Normalizer(const double h, const ODE_Fixed_Step_Handler handler, const _step_normalizer_mode mode, const Step_Normalizer_Bounds bounds) 
    {
        this.h          = std::abs(h);
    ...
    /** {@inherit_doc} */
    //override
    public void init(const ODE_State_And_Derivative initial_state, const double const_time) 
    {
    ...
    public void handle_step(const ODE_StateInterpolator interpolator) 
    {
        ...
        double next_time = (mode == _step_normalizer_mode.INCREMENT) ?
                          last.get_time() + h :
                          (std::floor(last.get_time() / h) + 1) * h;
        if (mode == _step_normalizer_mode.MULTIPLES &&
            Precision.equals(next_time, last.get_time(), 1)) 
            {
            next_time += h;
        }
    ...
    /** {@inherit_doc} */
    //override
    public void finish(ODE_State_And_Derivative const_state) 
    {
            ...
            bool add_last = bounds.last_included() &&
                              last.get_time() != const_state.get_time();
     ...
     * current step
     */
    private bool is_next_in_step(double next_time, ODE_StateInterpolator interpolator) 
    {
        return forward ?
               next_time <= interpolator.get_current_state().get_time() :
               next_time >= interpolator.get_current_state().get_time();
    }
     ...
     */
    private void do_normalized_step(bool is_last) 
    {
        if (!bounds.first_included() && first.get_time() == last.get_time()) 
        {
            return;
        }
