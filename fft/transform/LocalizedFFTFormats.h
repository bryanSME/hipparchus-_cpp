...
#include <string>
   ...
   * </p>
   */
class Localized_FFT_Formats : Localizable
{
...
private:
	 ...
	 */
	Localized_FFT_Formats(const std::string& source_format) : my_source_format{ source_format } {}
...
public:
	...
	/** {@inherit_doc} */
	//override
	std::string get_localized_string(const Locale& locale)
	{
		try
		{
			const std::string path = Localized_FFT_Formats.class.get_name().replace_all("\\.", "/");
		...
		catch (Missing_Resource_Exception mre)
		{
			// NOPMD
			// do nothing here
		}
