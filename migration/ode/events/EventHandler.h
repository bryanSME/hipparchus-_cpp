 ...
 * @deprecated as of 1.0, replaced with {@link ODE_Event_Handler}
 */
@Deprecated
class Event_Handler extends ODE_Event_Handler 
{
    ...
    /** {@inherit_doc} */
    //override
    default double g(const ODE_State_And_Derivative state) 
    {
        return g(state.get_time(), state.get_primary_state());
    }

    /** {@inherit_doc} */
    //override
    default org.hipparchus.ode.events.Action event_occurred(const ODE_State_And_Derivative state, const bool increasing) 
    {
        switch (event_occurred(state.get_time(), state.get_primary_state(), increasing)) 
        {
            case CONTINUE:
    ...
    /** {@inherit_doc} */
    //override
    default ODE_State reset_state(const ODE_State_And_Derivative state) 
    {
        const double   t = state.get_time();
        const std::vector<double> y = state.get_primary_state();
        reset_state(t, y);
        return ODE_State(t, y);
    }
    ...
    enum Action 
    {
         ...
         * #event_occurred event_occurred} method when the integration should be
         * stopped after the event ending the current step.</p>
         */
        STOP, 
        /** Reset state indicator.
         * <p>This value should be used as the return value of the {@link
         * #event_occurred event_occurred} method when the integration should
         * go on after the event ending the current step, with a state
         * vector (which will be retrieved thanks to the {@link #reset_state
         * reset_state} method).</p>
         */
        RESET_STATE, 
         ...
         * org.hipparchus.ode.Ordinary_Differential_Equation#compute_derivatives}
         * method).</p>
         */
        RESET_DERIVATIVES, 
        /** Continue indicator.
         * <p>This value should be used as the return value of the {@link
         * #event_occurred event_occurred} method when the integration should go
     ...
     * time {@link #event_occurred(double, std::vector<double>, bool) event_occurred} is called, * {@code sign} is reset to {@code -sign}. This allows the {@code g(t)}
     ...
     */
    double g(double t, std::vector<double> y);
     ...
     * org.hipparchus.migration.ode.First_Order_Differential_Equations
     ...
     *   will recompute the derivatives, *   <li>if {@link Action#CONTINUE} is returned, no specific action will
     ...
     */
    Action event_occurred(double t, std::vector<double> y, bool increasing);
     ...
     * step. If the {@link #event_occurred} never returns the {@link
     * Action#RESET_STATE} indicator, this function will never be called, and it is
     * safe to leave its body empty.</p>

     * @param t current value of the independent <i>time</i> variable
     * @param y array containing the current value of the state vector
     * the state should be put in the same array
     */
    void reset_state(double t, std::vector<double> y);
