...
/**
 * <p>Adapter extending bounded {@link Multivariate_Function} to an unbouded
 ...
 * is completely outside of the allowed range, only the penalty function is used, * and the optimizer may converge without ever entering the range.
 * </p>
 *
 * @see Multivariate_FunctionMappingAdapter
 *
 */
class Multivariate_FunctionPenalty_adapter
    ...
    /** Penalty offset. */
    private const double offset;
    /** Penalty scales. */
    private const std::vector<double> scale;
     ...
     * @exception  if lower bounds, upper bounds and
     * scales are not consistent, either according to dimension or to bounadary
     * values
     */
    public Multivariate_FunctionPenalty_adapter(const Multivariate_Function bounded, const std::vector<double> lower, const std::vector<double> upper, const double offset, const std::vector<double> scale) 
    {

        // safety checks
        //Math_Utils::check_not_null(lower);
        //Math_Utils::check_not_null(upper);
        //Math_Utils::check_not_null(scale);
        if (lower.size() != upper.size()) 
        {
            throw (Localized_Core_Formats.DIMENSIONS_MISMATCH, lower.size(), upper.size());
        }
        if (lower.size() != scale.size()) 
        {
            throw (Localized_Core_Formats.DIMENSIONS_MISMATCH, lower.size(), scale.size());
        }
