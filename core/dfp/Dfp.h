...
protected:
	 ...
	 */
	Dfp(const DFP_Field field, std::byte x)
	{
	 ...
	 * @param x value to convert to an instance
	 */
	Dfp(const DFP_Field& field, const int& x)
	{
		Dfp(field, static_cast<long>(x);
	}
	 ...
	 * @param x value to convert to an instance
	 */
	Dfp(const DFP_Field& field, const long& x)
	{
		// initialize as if 0
		mant = std::vector<int>(field.get_radix_digits());
		nans = FINITE;
		this.field = field;

		bool is_long_min{};
		if (x == std::numeric_limits<long>::MIN_VALUE())
		{
	 ...
	 * @param x value to convert to an instance
	 */
	Dfp(const DFP_Field& field, const double& x)
	{
		// initialize as if 0
		mant = int[field.get_radix_digits()];
		this.field = field;
	 ...
	 */
	Dfp(const DFP_Field& field, const std::string& s)
	{
		// initialize as if 0
		mant = int[field.get_radix_digits()];
		sign = 1;
	 ...
	 */
	Dfp(const DFP_Field field, const std::byte sign, const std::byte nans)
	{
		this.field = field;
	 ...
	 */
	Dfp trunc(const DFP_Field.Rounding_Mode rmode)
	{
		bool changed = false;
	 ...
	 *  @return ???
	 */
	int complement(const int& extra)
	{
		extra = RADIX - extra;
	 ...
	 */
	Dfp trap(const int& type, std::string what, Dfp oper, Dfp def, Dfp result)
	{
		return def;
	}
...
public:
	...
	static constexpr int RADIX{ 10000 };

	/** The minimum exponent before underflow is signaled.  Flush to zero
	 *  occurs at minExp-DIGITS */
	static constexpr int MIN_EXP{ -32767 };

	/** The maximum exponent before overflow is signaled and results flushed
	 *  to infinity */
	static constexpr int MAX_EXP{ 32768 };

	/** The amount under/overflows are scaled by before going to trap handler */
	static constexpr int ERR_SCALE{ 32760 };

	/** Indicator value for normal finite numbers. */
	static constexpr std::byte FINITE{ 0 };

	/** Indicator value for Infinity. */
	static constexpr std::byte INFINITE{ 1 };

	/** Indicator value for signaling NaN. */
	static constexpr std::byte SNAN{ 2 };

	/** Indicator value for quiet NaN. */
	static constexpr std::byte QNAN{ 3 };

private:
	/** std::string for NaN representation. */
	static const std::string NAN_STRING{ "NaN" };

	/** std::string for positive infinity representation. */
	static const std::string POS_INFINITY_STRING{ "Infinity" };

	/** std::string for negative infinity representation. */
	static const std::string NEG_INFINITY_STRING{ "-Infinity" };

	/** Name for traps triggered by addition. */
	static const std::string ADD_TRAP{ "add" };

	/** Name for traps triggered by multiplication. */
	static const std::string MULTIPLY_TRAP{ "multiply" };

	/** Name for traps triggered by division. */
	static const std::string DIVIDE_TRAP{ "divide" };

	/** Name for traps triggered by square root. */
	static const std::string SQRT_TRAP{ "sqrt" };

	/** Name for traps triggered by alignment. */
	static const std::string ALIGN_TRAP{ "align" };

	/** Name for traps triggered by truncation. */
	static const std::string TRUNC_TRAP{ "trunc" };

	/** Name for traps triggered by next_after. */
	static const std::string NEXT_AFTER_TRAP{ "next_after" };

	/** Name for traps triggered by less_than. */
	static const std::string LESS_THAN_TRAP{ "less_than" };

	/** Name for traps triggered by greater_than. */
	static const std::string GREATER_THAN_TRAP{ "greater_than" };

	/** Name for traps triggered by new_instance. */
	static const std::string NEW_INSTANCE_TRAP{ "new_instance" };

	/** Multiplication factor for number of digits used to compute linear combinations. */
	static constexpr int LINEAR_COMBINATION_DIGITS_FACTOR{ 2 };

	/** Factory building similar Dfp's. */
	const DFP_Field field;
	 ...
	 * @since 1.7
	 */
	static bool shouldIncrement(const DFP_Field.Rounding_Mode rmode, const bool zero_lsb, const int lastDiscarded, const int first_non_discarded, const int& sign)
	{
		switch (rmode)
		{
		case ROUND_DOWN:
	 ...
	 * @since 1.7
	 */
	static bool is_half_way(const bool zero_lsb, const int lastDiscarded)
	{
	 ...
	 * @since 1.7
	 */
	static bool is_above_half_way(const bool zero_lsb, const int lastDiscarded)
	{
	 ...
	 */
	static int compare(const Dfp& a, const Dfp& b)
	{
	 ...
	 */
	Dfp new_instance(const std::byte sig, const std::byte code)
	{
	 ...
	 * @since 1.7
	 */
	Dfp new_instance(const DFP_Field& target_field, const DFP_Field::Rounding_Mode& rmode)
	{
	 ...
	 */
	Dfp dotrap(const int& type, std::string what, Dfp oper, Dfp result)
	{
		Dfp def = result;
	 ...
	 */
	static Dfp copysign(const Dfp x, const Dfp y)
	{
	 ...
	 * @see #toDouble()
	 */
	std::vector<double> toSplitDouble()
	{
...

	{
		...
		else
		{
			...
			const Dfp pmPi = new_instance((tmp.sign <= 0)
				? -std::numbers::pi 
				: std::numbers::pi);
	 ...
	 */
	 //override
	Dfp linear_combination(const Dfp[] a, const Dfp[] b)
	 ...
	 */
	 //override
	Dfp linear_combination(const std::vector<double> a, const Dfp[] b)
	 ...
	 */
	 //override
	Dfp linear_combination(const Dfp& a1, const Dfp& b1, const Dfp& a2, const Dfp& b2)
	{
	 ...
	 */
	 //override
	Dfp linear_combination(const double& a1, const Dfp& b1, const double& a2, const Dfp& b2)
	{
		// switch to extended accuracy
	 ...
	 */
	 //override
	Dfp linear_combination(const Dfp& a1, const Dfp& b1, const Dfp& a2, const Dfp& b2, const Dfp a3, const Dfp& b3)
	{
		// switch to extended accuracy
		const DFP_Field extendedField = a1.field.get_extended_field(LINEAR_COMBINATION_DIGITS_FACTOR, false);
		const Dfp a1Ext = a1.new_instance(extendedField, NULL);
	 ...
	 */
	 //override
	Dfp linear_combination(const double& a1, const Dfp& b1, const double& a2, const Dfp& b2, const double& a3, const Dfp& b3)
	{
		// switch to extended accuracy
		const DFP_Field extendedField = b1.field.get_extended_field(LINEAR_COMBINATION_DIGITS_FACTOR, false);
		const Dfp b1Ext = b1.new_instance(extendedField, NULL);
	 ...
	 */
	 //override
	Dfp linear_combination(const Dfp& a1, const Dfp& b1, const Dfp& a2, const Dfp& b2, const Dfp a3, const Dfp& b3, const Dfp a4, const Dfp& b4)
	{
		// switch to extended accuracy
		const DFP_Field extendedField = a1.field.get_extended_field(LINEAR_COMBINATION_DIGITS_FACTOR, false);
		const Dfp a1Ext = a1.new_instance(extendedField, NULL);
	 ...
	 */
	 //override
	Dfp linear_combination(const double& a1, const Dfp& b1, const double& a2, const Dfp& b2, const double& a3, const Dfp& b3, const double& a4, const Dfp& b4)
	{
		// switch to extended accuracy
		const DFP_Field extendedField = b1.field.get_extended_field(LINEAR_COMBINATION_DIGITS_FACTOR, false);
		const Dfp b1Ext = b1.new_instance(extendedField, NULL);
