...
class Akima_Spline_Interpolator
	: 
	public Univariate_Interpolator,
	public Field_Univariate_Interpolator
{
private:
	/** The minimum number of points that are needed to compute the function. */
	static constexpr int MINIMUM_NUMBER_POINTS{ 5 };

	/** Weight modifier to avoid overshoots. */
	const bool my_use_modified_weights;
	 ...
	 */
	double differentiate_three_point(const std::vector<double>& xvals, const std::vector<double>& yvals, int index_of_differentiation, int index_of_first_sample, int index_of_second_sample, int index_of_third_sample)
	{
	...
	T differentiate_three_point(std::vector<T> xvals, std::vector<T> yvals, int index_of_differentiation, int index_of_first_sample, int index_of_second_sample, int index_of_third_sample)
	{
	 ...
	 */
	Polynomial_Spline_Function interpolate_hermite_sorted(const std::vector<double>& xvals, const std::vector<double>& yvals, std::vector<double> first_derivatives)
	{
		...
		auto polynomials = std::vector<Polynomial_Function>(size);
	 ...
	 */
	template<typename T, typename std::enable_if<std::is_base_of<Calculus_Field_Element<T>, T>::value>::type* = nullptr>
	Field_Polynomial_Spline_Function<T> interpolate_hermite_sorted(std::vector<T> xvals, std::vector<T> yvals, std::vector<T> first_derivatives)
	{
		Math_Arrays::check_equal_length(xvals, yvals);
		Math_Arrays::check_equal_length(xvals, first_derivatives);
...
public:
	 ...
	 * @since 2.1
	 */
	Akima_Spline_Interpolator()
	{
		Akima_Spline_Interpolator(false);
	}
	 ...
	 * @since 2.1
	 */
	Akima_Spline_Interpolator(const bool use_modified_weights)
		:
		my_use_modified_weights{ use_modified_weights }
	{
	}
	 ...
	 */
	 //override
	Polynomial_Spline_Function interpolate(const std::vector<double>& xvals, const std::vector<double>& yvals)
	{
		Math_Arrays::check_equal_length(xvals, yvals);
		...
		auto differences = std::vector<double>(number_of_diff_and_weight_elements];
		auto weights = std::vector<double>(number_of_diff_and_weight_elements];
		...
		auto first_derivatives = std::vector<double>(xvals.size());
	 ...
	 */
	 //override
	template<typename T, typename std::enable_if<std::is_base_of<Calculus_Field_Element<T>, T>::value>::type* = nullptr>
	Field_Polynomial_Spline_Function<T> interpolate(const std::vector<T>& xvals, const std::vector<T>& yvals)
	{
		Math_Arrays::check_equal_length(xvals, yvals);
