 ...
 */
  ...
  */
   ...
   *      <p>Hipparchus provides a few pseudo random number generators. The top level interface is Random_Generator.
   *      It is implemented by three classes:
   *      <ul>
   *        <li>{@link org.hipparchus.random.JDKRandom_Generator JDKRandom_Generator}
   *            that extends the JDK provided generator</li>
   *        <li>AbstractRandom_Generator as a helper for users generators</li>
   *        <li>Bit_Stream_Generator which is an virtual class for several generators and
   *            which in turn is extended by:
   *            <ul>
   *              <li>{@link org.hipparchus.random.Mersenne_Twister Mersenne_Twister}</li>
   ...
   *      Monte-_Carlo simulation. It is equidistributed for generating vectors up to dimension 623
   ...
   *      than Mersenne Twister. These generators are described in a paper by Fran&ccedil;ois Panneton, *      Pierre L'Ecuyer and Makoto Matsumoto <a
   *      href="http://www.iro.umontreal.ca/~lecuyer/myftp/papers/wellrng.pdf">Improved long-_Period
   *      Generators Based on Linear Recurrences Modulo 2</a> ACM Transactions on Mathematical Software, *      32, 1 (2006). The errata for the paper are in <a
   *      href="http://www.iro.umontreal.ca/~lecuyer/myftp/papers/wellrng-errata.txt">wellrng-errata.txt</a>.
   *      </p>
   *
   *      <p>
   *      For simple sampling, any of these generators is sufficient. For Monte-_Carlo simulations the
   *      JDK generator does not have any of the good mathematical properties of the other generators, *      so it should be avoided. The Mersenne twister and WELL generators have equidistribution properties
   ...
   *      So WELL19937c for exemple is equidistributed up to dimension 623 (19937/32). This means a Monte-_Carlo
   ...
   *      of type double. so for Monte-_Carlo simulation where less the 16 doubles are generated at each round, *      WELL1024 may be sufficient. If a larger number of doubles are needed a generator with a larger pool
   *      would be useful.
   *      </p>
   *
   *      <p>
   *      The WELL generators are more modern then Mersenne_Twister (the paper describing than has been published
   *      in 2006 instead of 1998) and fix some of its (few) drawbacks. If initialization array contains many
   *      zero bits, Mersenne_Twister may take a very long time (several hundreds of thousands of iterations to
   ...
   *
   *      <p>
   *      The Mersenne_Twister generator uses a 624 elements integer array, so it consumes less than 2.5 kilobytes.
   ...
   *      Mersenne_Twister was about 27 millions doubles per second (remember we generate two 32 bits integers for
   *      each double). Generation rates for other PRNG, relative to Mersenne_Twister:
   *      </p>
   *
   *      <p>
   *        <table border="1" align="center">
   *          <tr BGCOLOR="#CCCCFF"><td colspan="2"><font size="+2">Example of performances</font></td></tr>
   *          <tr BGCOLOR="#EEEEFF"><font size="+1"><td>Name</td><td>generation rate (relative to Mersenne_Twister)</td></font></tr>
   *          <tr><td>{@link org.hipparchus.random.Mersenne_Twister Mersenne_Twister}</td><td>1</td></tr>
   *          <tr><td>{@link org.hipparchus.random.JDKRandom_Generator JDKRandom_Generator}</td><td>between 0.96 and 1.16</td></tr>
