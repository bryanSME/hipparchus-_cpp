...
#include <cmath>
   ...
   * </p>
   * <p>
   * Implementation of {@link #inv_gamma1pm1static_cast<double>(} and
   * {@link #log_gamma1pstatic_cast<double>(} is based on the algorithms described in
   * <ul>
...
private:
	...
	static const std::vector<double> LANCZOS
	{
	...
	/** Avoid repeated computation of log of 2 PI in log_gamma */
	static const double HALF_LOG_2_PI = 0.5 * std::log(2.0 * std::numbers::pi);
	...
	/*
	 * Constants for the computation of double inv_gamma1pm1static_cast<double>(.
...
public:
	 ...
	 */
	static constexpr double GAMMA{ 0.577215664901532860606512090082 }; // NOPMD - the fact the function and the constant have the same name is intentional and comes from mathematics conventions

	/**
	 * The value of the {@code g} constant in the Lanczos approximation, see
	 * {@link #lanczosstatic_cast<double>(}.
	 */
	static const double LANCZOS_G{ 607.0 / 128.0 };
	 ...
	 *
	 * @param x Argument.
	 * @return the value of {@code log(Gamma(x))}, {@codeNAN} if
	 * {@code x <= 0.0}.
	 */
	static double log_gamma(const double& x)
	{
		...
		if (x < 0.5)
		{
			return log_gamma1p(x) - std::log(x);
		}
		if (x <= 2.5)
		{
		...
		if (x <= 8.0)
		{
			...
			return log_gamma1p(x - (n + 1)) + std::log(prod);
		}
		const auto sum = lanczos(x);
		const auto tmp = x + LANCZOS_G + .5;
		return ((x + .5) * std::log(tmp)) - tmp + HALF_LOG_2_PI + std::log(sum / x);
	}
	 ...
	 */
	static double regularized_gamma_p(const double& a, const double& x)
	{
		return regularized_gamma_p(a, x, DEFAULT_EPSILON, std::numeric_limits<int>::max());
	}
	 ...
	 * @Math_Illegal_State_Exception if the algorithm fails to converge.
	 */
	static double regularized_gamma_p(const double& a, const double& x, const double& epsilon, const int& max_iterations)
	{
		double ret;
		...
		if (x >= a + 1)
		{
			// use regularized_gamma_q because it should converge faster in this
			// case.
			return 1.0 - regularized_gamma_q(a, x, epsilon, max_iterations);
		}

		// calculate series
		double n{}; // current element index
		double an{ 1.0 / a }; // n-th element in the series
		double sum{ an }; // partial sum
		while (std::abs(an / sum) > epsilon && n < max_iterations && sum < INFINITY)
		{
		...
		return std::exp(-x + (a * std::log(x)) - log_gamma(a)) * sum;
	}
	 ...
	 * @Math_Illegal_State_Exception if the algorithm fails to converge.
	 */
	static double regularized_gamma_q(const double& a, double x)
	{
		return regularized_gamma_q(a, x, DEFAULT_EPSILON, std::numeric_limits<int>::max());
	}
	 ...
	 * @Math_Illegal_State_Exception if the algorithm fails to converge.
	 */
	static double regularized_gamma_q(const double& a, const double& x, const double& epsilon, const int& max_iterations)
	{
		if (std::isnan(a) || std::isnan(x) || (a <= 0.0) || (x < 0.0))
		{
			return std::numeric_limits<double>::quiet_NaN();
		...
		if (x < a + 1.0)
		{
			// use regularized_gamma_p because it should converge faster in this
			// case.
			return 1.0 - regularized_gamma_p(a, x, epsilon, max_iterations);
		}
		// create continued fraction
		Continued_Fraction cf = Continued_Fraction()
		{
			/** {@inherit_doc} */
			//override
		...
		return std::exp(-x + (a * std::log(x)) - log_gamma(a)) * (1.0 / cf.evaluate(x, epsilon, max_iterations));
	}
	 ...
	 * @see Gamma#digammastatic_cast<double>(
	 */
	 ...
	 * @ if {@code x > 1.5}
	 */
	static double inv_gamma1pm1(const double& x)
	{
		if (x < -0.5)
		{
			throw std::exception("not implemented");
			//throw (hipparchus::exception::Localized_Core_Formats_Type::NUMBER_TOO_SMALL, x, -0.5);
		}
		if (x > 1.5)
		{
			throw std::exception("not implemented");
			//throw (hipparchus::exception::Localized_Core_Formats_Type::NUMBER_TOO_LARGE, x, 1.5);
		}
	 ...
	 * @ if {@code x > 1.5}.
	 */
	static double log_gamma1p(const double& x)
	{
		if (x < -0.5)
		{
			throw std::exception("not implemented");
		...
		return -std::log1p(inv_gamma1pm1(x));
	}
	...
	static double gamma(const double& x)
	{
		...
		if (abs_x <= 20.0)
		{
			if (x >= 1.0)
			{
				 ...
				 * Gamma(x) = (x - 1) * ... * (x - n) * Gamma(x - n), * then
				 * Gamma(t) = 1 / [1 + inv_gamma1pm1(t - 1)], * where t = x - n. This means that t must satisfy
				 * -0.5 <= t - 1 <= 1.5.
				 */
				double prod{ 1.0 };
				double t{ x };
				...
				return prod / (1.0 + inv_gamma1pm1(t - 1.0));
			}
			/*
			 * From the recurrence relation
			 * Gamma(x) = Gamma(x + n + 1) / [x * (x + 1) * ... * (x + n)]
			 * then
			 * Gamma(x + n + 1) = 1 / [1 + inv_gamma1pm1(x + n)], * which requires -0.5 <= x + n <= 1.5.
			 */
			double prod{ x };
			double t{ x };
			while (t < -0.5)
			...
			return 1.0 / (prod * (1.0 + inv_gamma1pm1(t)));
		}

		const double y{ abs_x + LANCZOS_G + 0.5 };
		const double gamma_abs{ SQRT_TWO_PI / abs_x * std::pow(y, abs_x + 0.5) * std::exp(-y) * lanczos(abs_x) };
		if (x > 0.0)
		{
			return gamma_abs;
		}
		 ...
		 * Gamma(x) * Gamma(1 - x) * sin(pi * x) = pi, * and the recurrence relation
		 * Gamma(1 - x) = -x * Gamma(-x), * it is found
		 * Gamma(x) = -pi / [x * sin(pi * x) * Gamma(-x)].
		 */
		return -std::numbers::pi / (x * std::sin(std::numbers::pi * x) * gamma_abs);
	}
};