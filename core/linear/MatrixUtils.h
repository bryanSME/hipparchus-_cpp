...
private:
	 ...
	 */
	static bool is_symmetricInternal(const Real_Matrix& matrix, const double& relative_tolerance, bool raiseException)
	{
...
public:
	...
	static Real_Matrix create_real_matrix(const int& rows, const int& columns)
	{
		return (rows * columns <= 4096)
			? Array_2D_Row_Real_Matrix(rows, columns)
			: Block_Real_Matrix(rows, columns);
	}
	...
	{
		return rows * columns <= 4096
			? Array2DRowField_Matrix<T>(field, rows, columns)
			: BlockField_Matrix<T>(field, rows, columns);
	}
	 ...
	 */
	static <T extends Field_Element<T>> Field_Matrix<T> create_field_matrix(const std::vector<std::vector<T>>& data)
	{
		if (data == NULL || data[0] == NULL)
		{
			throw std::exception("not implemented");
	...
	{
		auto m = create_real_matrix(dimension, dimension);
	...
	static <T extends Field_Element<T>> Field_Vector<T> create_field_vector(const std::vector<T>& data)
	{
		...
		return Array_Field_Vector<T>(data[0].get_field(), data, true);
	}
	...
	{
		return Array_Field_Vector<>(Math_Arrays::build_array(field, dimension));
	}
	...
	static <T extends Field_Element<T>> Field_Matrix<T> createRowField_Matrix(const std::vector<T>& row_data)
	{
		...
		auto m = create_field_matrix(row_data[0].get_field(), 1, n_cols);
		for (int i{}; i < n_cols; ++i)
		{
			m.set_entry(0, i, row_data[i]);
		}
	...
	static Real_Matrix create_column_real__matrix(const std::vector<double>& column_data)
	{
		...
		auto m = create_real_matrix(n_rows, 1);
	...
	static <T extends Field_Element<T>> Field_Matrix<T> createColumnField_Matrix(const std::vector<T>& column_data)
	{
		...
		auto m = create_field_matrix(column_data[0].get_field(), n_rows, 1);
		for (int i{}; i < n_rows; ++i)
		{
			m.set_entry(i, 0, column_data[i]);
		}
	 ...
	 * @ if the matrix is not symmetric.
	 */
	static void checkSymmetric(const Real_Matrix& matrix, const double& eps)
	{
	 ...
	 * @return {@code true} if {@code matrix} is symmetric.
	 */
	static bool is_symmetric(const Real_Matrix& matrix, const double& eps)
	{
	...
	{
		if ((left.get_row_dimension() != right.get_row_dimension()) || (left.get_column_dimension() != right.get_column_dimension()))
		{
			throw std::exception("not implemented");
	...
	private static class FractionMatrix_converter : public DefaultField_Matrix_Preserving_Visitor<Fraction>
	{
	private:
		/** Converted array. */
	...
	public:
		...
		{
			DefaultField_Matrix_Preserving_Visitor<Fraction>(Fraction::ZERO);
		}
		...
		{
			my_data = std::vector<std::vector<double>>(rows, std::vector<double>(columns));
		}

		/** {@inherit_doc} */
		//override
		void visit(const int& row, const int& column, const Fraction& value)
		{
			my_data[row][column] = value.double_value();
		}
		...
		{
			return Array_2D_Row_Real_Matrix(my_data, false);
		}
	};
	...
	{
		const auto converter = Big_FractionMatrix_converter();
		m.walk_in_optimized_order(converter);
		return converter.get_convertedMatrix();
	}

	/** Converter for {@link Field_Matrix}/{@link Big_Fraction}. */
	private static class Big_FractionMatrix_converter : public DefaultField_Matrix_Preserving_Visitor<Big_Fraction>
	{
	private:
		/** Converted array. */
		...
		/** {@inherit_doc} */
		//override
		public void visit(const int& row, const int& column, const Big_Fraction& value)
		{
	...
	public static void deserialize_real__vector(const Object& instance, const std::string& fieldName, const Object_Input_Stream& ois)
	{
		try
		{
			...
			const auto data = std::vector<double>(n);
			for (int i{}; i < n; ++i)
			{
			...
			// set up the field
			const java.lang.reflect.Field f = instance.get_class().get_declaredField(fieldName);
	...
	/** Orthonormalize a list of vectors.
		...
		* @since 2.1
		*/
	public static std::vector<Real_Vector> orthonormalize(const std::vector<Real_Vector>& independent, const double& threshold, const Dependent_Vectors_Handler& handler)
	{
		// create separate list
		const auto basis = std::vector<Real_Vector>(independent);
	...
	public static std::vector<Field_Vector<T>> orthonormalize(const Field<T> field, const std::vector<Field_Vector<T>> independent, const T threshold, const Dependent_Vectors_Handler handler)
	{
		// create separate list
		const auto basis = Array_list<Field_Vector<T>>(independent);
