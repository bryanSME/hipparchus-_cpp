 ...
 */
  ...
  */
   ...
   *
   * <p>
   * {@link org.hipparchus.geometry.partitioning.BSP_Tree BSP trees}
   ...
   * org.hipparchus.geometry.partitioning.BSP_Tree BSP_Tree} class using
   * bool objects as the leaf nodes attributes to represent the
   ...
   *
   * <p>
   * Rather than simply associating the internal nodes with an hyperplane, * we consider <em>sub-hyperplanes</em> which correspond to the part of
   ...
   * it). sin_ce the parts are convex, the sub-hyperplanes are convex, in
   ...
   * corresponding child tree and to repeat the procedure recursively, * until either the point appears to be exactly on one of the hyperplanes
   * in the middle of the tree or to be in one of the leaf parts. For
   * this operation, it is sufficient to consider the complete hyperplanes, * there is no need to check the points with the boundary of the
