 ...
 */
  ...
  * @since 1.3
  */
class Kalman_Filter<T extends Measurement>
{
	 ...
	 * @exception Math_Runtime_Exception if estimation fails
	 */
	Process_Estimate estimation_step(T measurement)
		Math_Runtime_Exception;

	/** Get the current predicted state.
	 * @return current predicted state
	 */
	Process_Estimate get_predicted();

	/** Get the current corrected state.
	 * @return current corrected state
	 */
	Process_Estimate get_corrected();
}